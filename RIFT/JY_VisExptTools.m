...
switch fun_name
    ...
    case 'make_smooth_circular_mask'
    ...
    case 'visual_feedback'    
        ...
        if ~isfield(cfg,'correct')
            ...
            if all(~isfield(cfg,{'posSign','negSign','timedur'}))
                cfg.posSign = '+';
                cfg.negSign = '-';
                cfg.timedur = 0.5;
            end
        ...
        if cfg.correct == 0
            DrawFormattedText(wPtr, cfg.negSign, 'center', 'center', [255 0 0]);
        else
            DrawFormattedText(wPtr, cfg.posSign, 'center', 'center', [0 255 0]);
        end
        % Screen('Flip',wPtr); pause(cfg.timedur);
