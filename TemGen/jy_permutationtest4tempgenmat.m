...
cfg0.stattimewin      = ft_getopt( cfg0, 'stattimewin', [cfg0.traintime(1),cfg0.traintime(end)]);
...
cfg.avgoverfreq      = 'no';
cfg.frequency        = 'all';%cfg0.traintime; %'all';
cfg.avgovertime      = 'no';
cfg.latency          = cfg0.stattimewin; %'all';
outstat = ft_freqstatistics( cfg, s1, s2 ); %s1 - s2.


% compute t-val for all timepoints
cfg.frequency        = 'all'; 
cfg.latency          = 'all'; 
cfg.numrandomization = 1;
tmp = ft_freqstatistics( cfg, s1, s2 );
outstat.tval = tmp.stat;
